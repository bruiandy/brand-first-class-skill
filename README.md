# 品牌经营基本功 · AI Skill 系统

> 一套把品牌基本经营功拆成可路由、可追问、可收束的 AI Skill 系统。  
> 面向想把品牌判断做扎实、并用 AI 扩展自己经营能力的品牌经营者、品牌团队和顾问。
> 核心路径是：**人 + 经验 + AI = 个人品牌经营系统**。

---

## 这是什么？

这是《品牌第一课》课程内容的结构化 AI 封装版，包含 **12 个独立 Skill 模块**，可在支持 Skill 系统的 AI 工具（如 Codex、Claude Code、Antigravity 等）中直接调用。

每个 Skill 都包含：课程方法论精华 + 对话引导逻辑 + 章节路由指引，让 AI 真正"读懂"这套方法论，而不只是搜索关键词。

说明：对外网站可以使用 `6 个品牌经营基本功` 作为传播结构；Skill 系统不按网站结构走，而按课程产品本体走，也就是 `开篇 + 7 个章节`。

## 对外网站

这套 Skill 还有一版对外展示页，适合先看整体路径、六个基本功和案例入口。

[brandfirst.netlify.app](https://brandfirst.netlify.app/)

---

## 快速安装

### 方式一：克隆到本地使用

```bash
git clone https://github.com/bruiandy/brand-first-class-skill.git
```

克隆完成后，将 `skills/` 文件夹复制到你项目的 Skill 目录下即可：

```bash
cp -r brand-first-class-skill/skills/. 你的项目路径/.codex/skills/
```

如果你使用 Claude Code，则复制到 `.claude/skills/`：

```bash
cp -r brand-first-class-skill/skills/. 你的项目路径/.claude/skills/
```

### 方式二：手动引用单个 Skill

只需要某一节？直接把对应的 Skill 文件夹复制进去即可，例如只用品牌叙事那节：

```bash
cp -r brand-first-class-skill/skills/brand-course-05-brand-narrative-system 你的项目路径/.codex/skills/
```

Claude Code 用户把目标目录替换成 `.claude/skills/` 即可。

---

## 系统结构

共 **12 个 Skill 模块**，分三层架构：

```
brand-method-hub  ← 安装后先用这个入口，先看总览、Artifact 链和路由
品牌第一课｜从这里开始（品牌第一课-从这里开始）  ← 再进入课程路由
├── 课程层（8 节，按顺序递进）
│   ├── 01 · 开篇：做初创品牌必须有节奏感
│   ├── 02 · 第一节：盘资源，找到不公平优势
│   ├── 03 · 第二节：建立业务经营思维
│   ├── 04 · 第三节：品牌终极之问（VMV）
│   ├── 05 · 第四节：构建品牌叙事体系
│   ├── 06 · 第五节：产品价值塑造
│   ├── 07 · 第六节：产品×渠道闭环
│   └── 08 · 第七节：构建最小营销闭环
└── 支撑工具层（3 个随时可调用）
    ├── 品牌基础诊断（brand-foundation-diagnosis）
    ├── 案例拆解透镜（brand-case-lens）
    └── 品牌方法总控（brand-method-hub）
```

---

## 各模块说明

### 🗺 起始入口

| Skill | 功能 |
|-------|------|
| `brand-method-hub` | 如果你刚安装完，或者想先看整套系统怎么接力，先用这个入口。它会先帮你理解总览、Artifact 链和路由关系 |
| `品牌第一课-从这里开始` | 如果你已经有具体问题，但不知道该从哪一节切入，再用这个入口。它会先判断你的问题卡在哪，再路由到对应章节 |

> 入口 skill 的显示名会跟安装环境有关。若只显示目录名，就以目录名为准；如果看到 `brand-method-hub`，先从方法总控开始看全局。

### 📚 课程层

| Skill | 节次 | 核心方法论 |
|-------|------|-----------|
| `brand-course-01-rhythm` | 开篇 | 认知→战略→运营→业务，四步节奏感 |
| `brand-course-02-resource-scan` | 第一节 | 盘资源三维度 + 真优势三问 |
| `brand-course-03-operating-think` | 第二节 | 品类×渠道×团队×品牌×毛利，五维经营联动 |
| `brand-course-04-brand-ultimate-question` | 第三节 | 愿景使命价值观（VMV）实战化追问 |
| `brand-course-05-brand-narrative-system` | 第四节 | 初心→价值体系→视觉体系→服务体系，品牌一致性 |
| `brand-course-06-product-value-shaping` | 第五节 | JTBD + 产品价值 = 动人叙事 × 超预期体验 |
| `brand-course-07-channel-product-loop` | 第六节 | 反向渠道逻辑，四层产品×渠道匹配判断 |
| `brand-course-08-minimum-marketing-loop` | 第七节 | 单渠道+单人群+单卖点，最小可盈利闭环 |

### 🔧 支撑工具层

| Skill | 功能 |
|-------|------|
| `brand-foundation-diagnosis` | 互动式品牌诊断：真实案例 / 问卷筛选 / 合作支持，三种对话模式 |
| `brand-case-lens` | 案例拆解：资源 / 经营 / 品牌 / JTBD / 闭环，五种分析透镜 |
| `brand-method-hub` | 方法总控：查看课程工作流、Artifact 链和下一步路由 |

---

## 三种使用场景

## 建议使用顺序

### 第一步：先从这里开始

如果你是第一次安装，或者你只有一个总问题，不知道该问哪一节，**不要默认从第一节开始**。

先用：

```text
品牌第一课-从这里开始
```

它会先帮你判断：

- 你的问题更像卡在节奏、资源、经营、品牌、产品价值，还是营销闭环
- 这更像单点问题，还是系统性问题
- 下一步最适合进入哪一节

### 第二步：再进入具体章节

只有当入口已经帮你收窄问题后，再进入对应单节 skill 深挖。

### 场景一：筛选合作客户

调用 `brand-foundation-diagnosis` → 进入问卷筛选模式。

AI 会通过 5-6 个核心问题，判断潜在客户的阶段、资源禀赋、合作匹配度，输出是否适合深入合作的初步信号。

### 场景二：分析品牌案例

调用 `brand-case-lens` → 选择 1-2 个最适合的拆解镜头。

不强制全套分析，每次先问"这个案例最值得用哪个视角看"，避免把方法论硬套。

### 场景三：合作过程中的支持

调用对应章节 Skill → 结合客户真实情况，给出阶段性建议、备忘录或下一步行动清单。

### 场景四：检查方法链路

调用 `brand-method-hub` → 检查你现在已有哪几份沟通纪要，判断下一步应该进入哪一节。

---

## 核心设计原则

1. **先听懂再问，先问清再分析** — 不急着给方案，先把问题搞清楚
2. **一次只问一个问题** — 避免问卷式轰炸，保持对话节奏
3. **区分已知事实 / 判断推测 / 仍需确认** — 不把推断当事实
4. **不做什么和做什么同样重要** — 每次输出都包含"暂时不要做"的建议
5. **章节间有明确路由** — 每节结束后，明确指向下一个最该看的节点

---

## 适用人群

- **品牌顾问** — 客户筛选、合作过程支持、案例方法论对齐
- **初创品牌创始人** — 自我诊断、阶段判断、找到最该做的那一步
- **品牌团队** — 方法论对齐、内部培训、案例复盘

---

## 关于《品牌第一课》

《品牌第一课》是一套面向**想把品牌基本功做扎实、并借助 AI 扩展经营能力的人**的实战方法论系统。

不讲概念堆砌，只讲品牌经营里每个阶段**最该做什么、最不该做什么、为什么这么做**。

**方法论主轴：**

```
校准节奏 → 盘资源 → 看经营 → 品牌终极之问 → 建品牌叙事 → 塑造产品价值 → 产品渠道闭环 → 跑通营销闭环
```

---

## License

MIT License — 欢迎在标注来源的前提下自由使用和修改。

---

*作者：bruiandy · 由 Antigravity AI 协助整理和封装*
